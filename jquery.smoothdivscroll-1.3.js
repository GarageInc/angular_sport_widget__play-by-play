...
(function ($) {
	...
	$.widget("thomaskahn.smoothDivScroll", {
		...
		options: {
			...
			hotSpotMouseDownSpeedBooster: 2, // Integer
			...
			mousewheelScrollingStep: 40, // Pixels
		...
		_create: function () {
					...
					el.data("rightScrollingInterval", setInterval(function () {
					    if (el.data("scrollXPos") > 0 && el.data("enabled")) {
						    ...
						    el.data("scrollWrapper")
                                .scrollLeft(el.data("scrollWrapper").scrollLeft() + (el.data("speedBooster")));
			...
			el.data("scrollingHotSpotLeft").bind("mouseover", function () {
				if (o.hotSpotScrolling) {
					...
					el.data("leftScrollingInterval", setInterval(function () {
					    if (el.data("scrollXPos") > 0 && el.data("enabled")) {
					        //console.log((el.data("scrollXPos") * el.data("speedBooster")));
			...
			*****************************************/
			...
			self.scrollToStartElement();

			self.disabledForScrollByTimer = false;

            $(document).on("moveToLastTime", {}, function (event, arg1, arg2) {
                if (!self.disabledForScrollByTimer) {
                    self.scrollToStartElement();
                }
            });
            ...
            el.data("scrollWrapper").bind("mouseover", function () {
                self.disabledForScrollByTimer = true;
		...
		scrollToStartElement: function () {
            var self = this, o = this.options, el = this.element;

            if (o.startAtElementId && o.startAtElementId !== '') {

                var scrollTo = $('#' + o.startAtElementId);
                if (scrollTo && scrollTo.length > 0) {

                    self.scrollToElement("id", o.startAtElementId);
                }
            }
        },
		...
		**********************************************************/
		...
		hideHotSpotBackgrounds: function (fadeSpeed) {
			...
			if (fadeSpeed !== undefined) {
			    ...
			    var dtL = el.data("scrollingHotSpotLeft");
			    var dtR = el.data("scrollingHotSpotRight");

			    if (dtL) {
                    // Fade out the left hotspot
			        dtL.fadeTo(fadeSpeed, 0.0, function () {
			            dtL.removeClass("scrollingHotSpotLeftVisible");
                    });
                }

			    if (dtR) {
			        // Fade out the right hotspot
			        dtR.fadeTo(fadeSpeed, 0.0, function () {
			            dtR.removeClass("scrollingHotSpotRightVisible");
			        });
			    }
			}
		...
		_showHideHotSpots: function () {
			...
			if (!(o.hotSpotScrolling)) {
				...
				// Regular hot spot scrolling.
					...
					// should be hidden
				    if (el.data("scrollWrapper") && (el.data("scrollableAreaWidth") <= (el.data("scrollWrapper").innerWidth()))) {
						//el.data("scrollingHotSpotLeft").hide();
						//el.data("scrollingHotSpotRight").hide();
					}
					// When you can't scroll further left the left scroll hotspot should be hidden
					// and the right hotspot visible.
				    else if (el.data("scrollWrapper") && el.data("scrollWrapper").scrollLeft() === 0) {
						el.data("scrollingHotSpotLeft").hide();
						el.data("scrollingHotSpotRight").show();
						// Callback
				    ...
				    else if (el.data("scrollWrapper") && el.data("scrollableAreaWidth") <= (el.data("scrollWrapper").innerWidth() + el.data("scrollWrapper").scrollLeft())) {
						el.data("scrollingHotSpotLeft").show();
						//el.data("scrollingHotSpotRight").hide();
						// Callback
				    ...
				    else if (el.data("scrollingHotSpotLeft") && el.data("scrollingHotSpotRight")) {
						el.data("scrollingHotSpotLeft").show();
						el.data("scrollingHotSpotRight").show();
					}
				}
		...
		_setElementScrollPosition: function (method, element) {
			...
			switch (method) {
			    ...
			    case "id":
			        ...
			        if (element.length > 0) {
			            if (el.data("scrollableArea").has("#" + element) && $("#" + element)) {
			                tempScrollPosition = parseInt($("#" + element)[0].style.left, 10);
							el.data("scrollXPos", tempScrollPosition);
							return true;
						}
					}
